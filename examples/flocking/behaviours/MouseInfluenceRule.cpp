...
Vector2f MouseInfluenceRule::computeForce(const std::vector<Boid*>& neighborhood, Boid* boid) {
  ImGui::SetCurrentContext(world->engine->window->imGuiContext);
      ...
      if (ImGui::IsMousePosValid() && io.MouseDown[0]) {
          ...
          // Gets distance to the mouse target
          Vector2f direction = mousePos - boid->getPosition();
          float distance = direction.getMagnitude();

          // Gets the current speed of the boid
          float speed = boid->getVelocity().getMagnitude();


          //The force is inversely proportional to distance
          Vector2f force = distance * speed * direction;
